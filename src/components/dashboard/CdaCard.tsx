...
interface CdaCardProps {
  cda: ExtendedCdaData;
  streets: ExtendedStreetData[];
  onStreetClick: (streetId: number) => void;
  onEdit: (cda: ExtendedCdaData) => void;
  onDelete: (cdaId: number) => void;
  onEditStreet: (street: ExtendedStreetData) => void;
...
export const CdaCard = ({ cda, streets, onStreetClick, onEdit, onDelete, onEditStreet, onDeleteStreet }: CdaCardProps) => {
  ...
  // Calculate total properties for this CDA - FIXED
            ...
            <div className="border-t border-border pt-4">
              <div className="space-y-3">
                {displayedStreets.map((street) => {
                  // Use safe property count for each street
                  const propertyCount = getPropertyCount(street);
                  const streetTotalProperties = propertyCount.houses + propertyCount.shops + propertyCount.hotels + propertyCount.others;
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-3 flex-shrink-0">
                        <div className="text-left sm:text-right text-sm">
                          <div className="font-medium text-foreground">{streetTotalProperties}</div>
